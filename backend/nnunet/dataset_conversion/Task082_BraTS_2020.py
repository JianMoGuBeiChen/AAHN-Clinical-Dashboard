...
if __name__ == "__main__":
    ...
    # Point these to YOUR scratch folders
    downloaded_data_dir = "/scratch/garima.jra.cse23.itbhu/AAHN_Project/BraTS2020_Data/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData"
    downloaded_data_dir_val = "/scratch/garima.jra.cse23.itbhu/AAHN_Project/BraTS2020_Data/BraTS2020_ValidationData/MICCAI_BraTS2020_ValidationData"    
    # This one can stay None if you don't have the test set zip, the script will skip it safely
    ...
    for p in subdirs(cur, join=False):
        ...
        # Use symlinks instead of shutil.copy to save disk space
        if not isfile(join(target_imagesTr, patient_name + "_0000.nii")):
            os.symlink(t1, join(target_imagesTr, patient_name + "_0000.nii"))
        if not isfile(join(target_imagesTr, patient_name + "_0001.nii")):
            os.symlink(t1c, join(target_imagesTr, patient_name + "_0001.nii"))
        if not isfile(join(target_imagesTr, patient_name + "_0002.nii")):
            os.symlink(t2, join(target_imagesTr, patient_name + "_0002.nii"))
        if not isfile(join(target_imagesTr, patient_name + "_0003.nii")):
            os.symlink(flair, join(target_imagesTr, patient_name + "_0003.nii"))
        
        # For the labels, we still use the conversion function as it modifies the data
        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsTr, patient_name + ".nii"))
    ...
    json_dict['numTest'] = 0
    json_dict['training'] = [{'image': "./imagesTr/%s.nii" % i, "label": "./labelsTr/%s.nii" % i} for i in
                             patient_names]
    json_dict['test'] = []
    ...
    if downloaded_data_dir_val is not None:
        ...
        for p in subdirs(downloaded_data_dir_val, join=False):
            ...
            # Use symlinks instead of shutil.copy to save disk space
            if not isfile(join(target_imagesVal, patient_name + "_0000.nii")):
                os.symlink(t1, join(target_imagesVal, patient_name + "_0000.nii"))
            if not isfile(join(target_imagesVal, patient_name + "_0001.nii")):
                os.symlink(t1c, join(target_imagesVal, patient_name + "_0001.nii"))
            if not isfile(join(target_imagesVal, patient_name + "_0002.nii")):
                os.symlink(t2, join(target_imagesVal, patient_name + "_0002.nii"))
            if not isfile(join(target_imagesVal, patient_name + "_0003.nii")):
                os.symlink(flair, join(target_imagesVal, patient_name + "_0003.nii"))
        
        # For the labels, we still use the conversion function as it modifies the data
        copy_BraTS_segmentation_and_convert_labels(seg, join(target_labelsVal, patient_name + ".nii"))


    # downloaded_data_dir_test = "/home/fabian/Downloads/MICCAI_BraTS2020_TestingData"

    if downloaded_data_dir_test is not None and isdir(downloaded_data_dir_test):
        for p in subdirs(downloaded_data_dir_test, join=False):
            ...
            shutil.copy(t1, join(target_imagesTs, patient_name + "_0000.nii"))
            shutil.copy(t1c, join(target_imagesTs, patient_name + "_0001.nii"))
            shutil.copy(t2, join(target_imagesTs, patient_name + "_0002.nii"))
            shutil.copy(flair, join(target_imagesTs, patient_name + "_0003.nii"))
